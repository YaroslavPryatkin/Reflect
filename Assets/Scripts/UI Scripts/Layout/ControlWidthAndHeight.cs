...
public class ControlWidthAndHeight : MonoBehaviour
{
    ...
    private void UpdateLayout()
    {
        ...
        var finalWidth = _currentParentRect.rect.width * fractions.x + staticChange.x;
        var finalHeight = _currentParentRect.rect.height * fractions.y + staticChange.y;

        if(shouldInvert)
            _rectTransform.sizeDelta = new Vector2(finalHeight, finalWidth);
        else
