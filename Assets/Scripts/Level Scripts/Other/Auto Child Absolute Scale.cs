...
public class AutoChildAbsoluteScale : MonoBehaviour
{
    ...
    public void AdjustScale(Transform parent)
    {
        ...
        {
            newLocalScale.x = targetX / _lastParentScale.x;
        }

        if (useTargetY && Mathf.Abs(_lastParentScale.y) > 0.0001f)
        {
            newLocalScale.y = targetY / _lastParentScale.y;
        }

        if (useTargetZ && Mathf.Abs(_lastParentScale.z) > 0.0001f)
        {
            newLocalScale.z = targetZ / _lastParentScale.z;
        }
...
#if UNITY_EDITOR
[CustomEditor(typeof(AutoChildAbsoluteScale))]
public class AutoChildAbsoluteScaleEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.LabelField("Absolute World Scale Modifiers", EditorStyles.boldLabel);
        EditorGUILayout.LabelField("Turn on the axis and set absolute size:", EditorStyles.miniLabel);
        EditorGUILayout.Space(5);
        ...
        {
            (target as AutoChildAbsoluteScale)?.UpdateAnyway();
        }
